...
foreach ($jsonData as $location) {
    ...
    foreach ($location['programs'] as $program) {
        ...
        echo '<div class="session" data-venue="'.$venueHash.'"  data-start="'.date('Y-m-d\TH:i', strtotime($program['startTime'])).'" data-end="'.date('Y-m-d\TH:i', strtotime($program['endTime'])).'"  style="grid-column: venue-' . $venueHash . '; grid-row: time-' . $start_time . ' / time-' . $end_time . ';">' . PHP_EOL;
